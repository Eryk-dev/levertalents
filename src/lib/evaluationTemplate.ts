...
export type TemplateQuestion =
  ...
  | { id: string; label: string; type: 'scale_1_3'; required: boolean }
...
function buildQuestionSchema(q: TemplateQuestion): ZodTypeAny {
  switch (q.type) {
    ...
    case 'scale_1_3': {
      const base = z.number().int().min(1).max(3);
      return q.required ? base : base.optional();
    }
    case 'text': {
...
export const templateSnapshotSchema = z.object({
  ...
  sections: z.array(z.object({
    ...
    questions: z.array(z.discriminatedUnion('type', [
      ...
      z.object({ id: z.string(), label: z.string(), type: z.literal('scale_1_3'), required: z.boolean() }),
