...
export function CycleResultsDrawer({ cycle, open, onOpenChange }: CycleResultsDrawerProps) {
  ...
  const userIds = useMemo(() => {
    ...
    for (const e of evaluations) {
      set.add(e.evaluator_user_id);
      set.add(e.evaluated_user_id);
    }
              ...
              {evaluationsQuery.isLoading ? (
                ...
                <div className="rounded-md border border-dashed border-border bg-bg-subtle/40 px-4 py-10 text-center">
                  ...
                  <p className="text-sm text-text-muted mt-1">
                    Quando os participantes enviarem suas avaliações, os resultados aparecem aqui.
                  </p>
                </div>
              ) : (
                <>
                      ...
                      <header className="flex items-baseline justify-between mb-3">
                        ...
                        {overallAvg !== null && (
                          <span className="text-[20px] font-semibold tabular-nums text-text">
                            ...
                            <span className="text-[12px] text-text-muted font-normal ml-1">/ 5</span>
                          </span>
                        )}
                      </header>
                        ...
                        {scaleAverages.map(({ question, avg, count }) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <p className="text-[13px] text-text truncate">{question.label}</p>
